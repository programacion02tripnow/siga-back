...
<div style="display: none;">Solicitud de liquidación con transferencia</div>
                            ...
                            <table style="font-family: 'Montserrat',Arial,sans-serif; width: 100%;" width="100%"
                                ...
                                <tr>
                                    <td class="sm-px-24"
                                        ...
                                        <p style="font-weight: 600; font-size: 18px; margin-bottom: 0;">Se ha solicitado
                                            una liquidación</p>
                                        <p class="sm-leading-32"
                                           style="font-weight: 600; font-size: 20px; margin: 0 0 24px; --text-opacity: 1; color: #263238; color: rgba(38, 50, 56, var(--text-opacity));">
                                            El método de liquidación es transferencia por un monto de
                                            <b>$ {{number_format($settlement->reservation_detail->net_price)}}</b>
                                            para la reserva {{$settlement->reservation_detail->provider_confirmation_number}}
                                        </p>
                                        ...
                                        <p style="margin: 24px 0; text-align: center">
                                            Los datos del proveedor para la transferencia son:
                                            <br>
                                            Proveedor: {{$settlement->reservation_detail->provider->name}}
                                            <br>
                                            Banco: {{$settlement->reservation_detail->provider->bank}}
                                            <br>
                                            Razón social: {{$settlement->reservation_detail->provider->business_name}}
                                            <br>
                                            RFC: {{$settlement->reservation_detail->provider->RFC}}
                                            <br>
                                            Clabe interbancaria: {{$settlement->reservation_detail->provider->clabe}}
                                        </p>
                                        <p style="margin: 24px 0; text-align: center">
                                            Después de realizar la transferencia, favor de confirmar la liquidación con
                                            el siguiente botón:
                                        </p>
                                        <a class="btn btn-info btn-lg"
                                           href="{{url('/auth_settlement/'.$settlement->id.'/'.$settlement->auth_token)}}">
                                            Confirmar
                                        </a>
